export default function Delivery() {
  return (
    
    <section className="flex flex-row justify-center bg-[#c9a227] py-20 text-white pt-40 pb-24">
      
      <div className="flex flex-col items-center justify-center mx-auto max-w-6xl px-4 md:px-8 gap-2">
        <h2 className="text-4xl font-semibold leading-tight tracking-wide md:text-3xl">
          Доставка из Англии
        </h2>
        <p className="text-sm  leading-tight tracking-wide md:text-base">14-17 дней до Москвы</p>
        <p className="text-sm leading-light md:text-base text-center">В среднем доставка товара после оплаты и обработки заказа из Великобритании до г. Москва составляет 14 дней. Данный срок может увеличиваться в праздничные дни, поэтому заранее предусматривайте возможные периоды загруженности логистики перед покупками.</p>
      </div>
    </section>
  );
} 